...
def convert_coordinates_to_original(cropped_coordinates, crop_box):
    """
    将裁剪后图像中的物品坐标转换为原始图像中的坐标。
    
    Args:
    - cropped_coordinates: 裁剪后图像中物品的坐标的元组 (x, y)。
    - crop_box: 裁剪框的左上角和右下角坐标的元组 ((x1, y1), (x2, y2))。
    ...
    # 将裁剪后图像中的坐标转换为相对于裁剪框左上角的坐标
    relative_x = cropped_x
    relative_y = cropped_y
    
    # 计算原始图像中的坐标
    original_x = int((relative_x / crop_width) * 1940)
    original_y = int((relative_y / crop_height) * 1080)
...
def main():
    ...
    while True:
        ...
        if flag3:
            cvui.text(window, 680, 480, "waiting start", 1)
        if button_clicked:
            ...
            # 获取物体的边界框信息
                    ...
                    for bbox, c ,conf in zip(r.boxes.xyxy, r.boxes.cls,r.boxes.conf):
                        if conf > 0.6:
                            ...
                            name = ("" if id is None else f"") + names[class_index]
                            label = (f"{name}")
                    ...
                    #y2=y2+desk_y1
                            ...
                            if y_ > 0:
                                ...
                                if label in object_counts:
                                    ...
                                    object_counts[label] = 1
                                if depth_values:
                                    average_depth_all = np.mean(depth_values)
                                else:
                                    average_depth_all = 0  # 如果列表为空，平均深度为0
   
                                all_frame_results.append(object_counts)
                                i = 0
                                for object_id, count in object_counts.items():
                                    cvui.text(window, 700,60 + 20*i, f"Goal_ID={object_id};Num={count}", 0.5)  # 在指定位置显示文本
                                    i+=1
                                max_results = max(all_frame_results, key=lambda x: sum(x.values()))
 
                                with open("result_r/detection_results.txt", "w") as file:
                             # 写入文件头
                                    file.write("START\n")
                             # 写入最多数量的种类及其数量
